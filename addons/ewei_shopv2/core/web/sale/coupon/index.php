...
//        ini_set('error_reporting', E_ALL);
        ...
        // 判断 获取到的 $_GPC 搜索字段 是否有值
        if (!empty($_GPC['keyword'])) {
            $_GPC['keyword'] = trim($_GPC['keyword']);
            // 拼接 sql 语句条件 ' uniacid = :uniacid and merchid=0 and isfriendcoupon = 0 AND couponname LIKE :couponname';
            $condition .= ' AND couponname LIKE :couponname';
            // 拼接 sql 语句参数 array(':uniacid' => $_W['uniacid'] ':couponname' => %搜索字段%);
    ...
    protected function post() {
        ...
        if ($_W['ispost']) {
            ...
            $data = array(
                ...
                'url_limit' => intval($_GPC['url_limit']),
