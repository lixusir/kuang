	...
	function main() {
		...
		if (!empty($year) && !empty($month) && !empty($day)) {
		    ...
		    for ($hour = 0; $hour < 24; $hour++) {
                ...
                $orderInfo =  pdo_fetchall("SELECT refundstate,id,price FROM " . tablename('ewei_shop_order') . " WHERE uniacid=:uniacid and status>=1 and isparent=0 and refundstate <>-1 and createtime >=:starttime and createtime <=:endtime ", array(
                        ':uniacid' => $_W['uniacid'],
                ...
                $dr = array(
                    ...
                    'count' => empty($type) ? array_sum(array_column($orderInfo,'price')) : count($orderInfo)
                );
                ...
                if (empty($type)){
                    foreach ($orderInfo as $item){
                        ...
                        }elseif ($item['refundstate'] == 3){

                            $refundPrice = pdo_fetchcolumn(" SELECT sum(applyprice) FROM ". tablename('ewei_shop_order_single_refund'). " WHERE uniacid=:uniacid AND status=1 AND orderid=:orderid",array(':uniacid'=>$_W['uniacid'],'orderid'=>$item['id']));

                            $dr['count'] -= $refundPrice;
                        }
		   ...
		   //todo
		    ...
		    foreach ($months as $k=>$m) {
                ...
                $orderInfo =  pdo_fetchall("SELECT refundstate,id,price FROM " . tablename('ewei_shop_order') . " WHERE uniacid=:uniacid and status>=1 and isparent=0 and refundid=0 and createtime >=:starttime and createtime <=:endtime GROUP BY id", array(
                        ':uniacid' => $_W['uniacid'],
