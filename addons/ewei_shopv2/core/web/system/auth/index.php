...
class Index_EweiShopV2Page extends SystemPage
{
	public function main()
	{
		...
		$modname = 'ewei_shopv2';
		$resp = ihttp_request(HTTP_X_FOR .'me.sy138.cn/app/ewei.php?a=term', array('ip' => $ip, 'modname' => $modname, 'code' => $_GPC['code'], 'domain' => $domain));
		$term = $resp['content'];
		$auth = get_auth();
		...
		if ($_W['ispost']) {
			...
			if(empty($sets) || !is_array($sets) || $_GPC['code']!=$sets['auth']['code']){
				$resp = ihttp_request(HTTP_X_FOR .'me.sy138.cn/app/ewei.php?a=rr', array('ip' => $ip, 'modname' => $modname, 'code' => $_GPC['code'], 'domain' => $domain));
				$result = @json_decode($resp['content'], true);
				
				if ($result['status'] == '1') {
					$sets['auth'] = array('ip' => $ip, 'modname' => $modname, 'code' => $_GPC['code'], 'domain' => $domain,'files'=>'/core/ping.php');
					if (empty($set)) {
				...
				if($_GPC['files']){
					$sets['auth']['files'] = implode(',',array_unique(array_filter(explode("\r\n", trim($_GPC['files'])))));
					if (empty($set)) {
						pdo_insert('ewei_shop_sysset', array('sets' => iserializer($sets), 'uniacid' => $_W['uniacid']));
					}else {
						pdo_update('ewei_shop_sysset', array('sets' => iserializer($sets)), array('id' => $set['id']));
