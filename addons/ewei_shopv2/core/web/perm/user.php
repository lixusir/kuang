...
class User_EweiShopV2Page extends WebPage {
	...
	function main() {
		...
		$list = pdo_fetchall("SELECT u.*,r.rolename FROM " . tablename('ewei_shop_perm_user') . " u  "
		...
		$total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('ewei_shop_perm_user') . " u  "
			. " left join " . tablename('ewei_shop_perm_role') . " r on u.roleid =r.id  "
	...
	protected function post() {
		...
		if ($_W['ispost']) {
			...
			if (!empty($item['id'])) {
                ...
                }elseif (function_exists('user_related_update')){
                    user_related_update($item['uid'],array('uid' => $item['uid'], 'password' => $_GPC['password'], 'salt' => $user['salt']));
                }else{
				    show_json(0,'参数异常，请联系开发人员');
                }
				...
				if (user_check(array('username' => $data['username']))) {
                    ...
                    $ret = in_array($user['uid'], $founders);
                    if ($ret === true){
                        show_json(0,'您已经是超级管理员了,请勿进行操作!!');
                    }
					$data['uid'] = $user['uid'];
