        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 AND ( o.status = 3 and (isverify=1 or o.istrade=1)) and o.verifyopenid = :openid' || !empty($sqlcondition)) {
            if ($this->merchPluginExists()) {
                $sql = "select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress, d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,sm.id as salerid,sm.nickname as salernickname,s.salername,
                  r.rtype,r.status as rstatus,o.sendtype,0 as isverifygoods_order from " . tablename('ewei_shop_order') . " o"
                    ...
                    . " left join " . tablename('ewei_shop_merch_saler') . " ms on ms.openid = o.verifyopenid and ms.uniacid=o.uniacid"
            ...
            if ($this->merchPluginExists()) {
                $sql = "SELECT o.*,0 as isverifygoods_order FROM" . tablename("ewei_shop_order") . " o "
                    . " LEFT JOIN " . tablename("ewei_shop_saler") . " s ON o.verifyopenid=s.openid"
                    . " LEFT JOIN " . tablename("ewei_shop_merch_saler") . " ms ON o.verifyopenid=ms.openid"
...
//                }
    ...
    public function orderDataVerify()
    {
        ...
        //当前核销员
        $saler = pdo_fetch('select id,openid from ' . tablename('ewei_shop_saler') . ' where status=1  and openid=:openid and uniacid=:uniacid limit 1', array(
...
//        订单号，核销员，核销门店，核销码，会员姓名，联系人，快递单号，商品名称，商品编码
        ...
        if ($condition != ' log.salerid = :salerid and log.uniacid = :uniacid ' || !empty($sqlcondition)) {
            ...
            $sql = "select o.* ,1 as isverifygoods_order,g.id as verifygoodsid from " . tablename('ewei_shop_verifygoods_log') . " log "
                . " left join  " . tablename('ewei_shop_verifygoods') . " g on log.verifygoodsid = g.id "
                . " left join  " . tablename('ewei_shop_order') . " o on g.orderid = o.id  "
                . " left join " . tablename('ewei_shop_member') . " m on m.openid=o.openid and m.uniacid =  o.uniacid "
            ...
            $sql = "select o.* ,1 as isverifygoods_order,g.id as verifygoodsid from " . tablename('ewei_shop_verifygoods_log') . " log "
                . " left join  " . tablename('ewei_shop_verifygoods') . " g on log.verifygoodsid = g.id "
                . " left join  " . tablename('ewei_shop_order') . " o on g.orderid = o.id where log.uniacid = :uniacid and log.salerid=:salerid GROUP BY o.id ORDER BY log.verifydate DESC  ";
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)) {
            ...
            $t = pdo_fetch(
                'SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('ewei_shop_order') . " WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0", $paras);
        }
    ...
    protected function selectOrderPrice($day = 0)
    {
        ...
        if ($this->merchPluginExists()) {
            ...
            $sql = "SELECT count(o.id) FROM" . tablename("ewei_shop_order") . " o"
                . " LEFT JOIN " . tablename("ewei_shop_saler") . " s ON o.verifyopenid=s.openid"
                . " LEFT JOIN " . tablename("ewei_shop_merch_saler") . " ms ON o.verifyopenid=ms.openid"
...
//            ':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']
//        ));
//        $merch_saler =  pdo_fetchall('select id,openid from ' . tablename('ewei_shop_merch_saler') . ' where status=1  and openid=:openid and uniacid=:uniacid limit 1', array(
//            ':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']
//        ));

//        $param['salerid'] = $saler['id'];
        $sql = "select count(*) from " . tablename('ewei_shop_saler_verify_log') . " where uniacid = :uniacid and openid = :openid  and  verify_time > :createtime1 and verify_time < :createtime2 and type = 0";
        // 判断 merch_saler表是否存在
...
//        $sql= "SELECT sum(verifynum) FROM". tablename("ewei_shop_verifygoods_log")."    WHERE
//          salerid=:salerid and verifydate between :createtime1 and :createtime2 and uniacid=:uniacid limit 1";
        $sql = "select count(*) from " . tablename('ewei_shop_saler_verify_log') . " where uniacid = :uniacid and openid = :openid  and  verify_time > :createtime1 and verify_time < :createtime2 and type = 1";
