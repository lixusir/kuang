...
//        m('message')->sendTplnotice($merch['openid'], $template['templateid'], $msg,'',$account);
    }
...
//					));
    ...
    public function sendOrderWillVerifyMessage($orderid = '0', $time)
    {
        global $_W, $_GPC;
        ...
        $orders = pdo_fetch("select g.type,o.id,o.openid,g.title,og.total from " . tablename('ewei_shop_order') .
            " o LEFT JOIN".tablename('ewei_shop_order_goods')." og on og.orderid=o.id
        LEFT JOIN ".tablename('ewei_shop_goods'). " g on g.id=og.goodsid 
        LEFT JOIN ".tablename('ewei_shop_verifygoods')." vg on vg.orderid=o.id where  o.uniacid={$_W['uniacid']} and o.id={$orderid} and willcloseverifymessage <>1");
        ...
        pdo_update('ewei_shop_order', array("willcloseverifymessage" => 1), array('id' => $orderid));

        $member = m('member')->getMember($orders['openid']);


        $time = date('Y-m-d H:i:s',$time);

        $datas = array(
            array("name" => "商品名称", "value" => $orders['title']),
            array("name" => "核销时间", "value" => $time)
        );

        $remark = "\n请尽快处理，逾期将作废，如有疑问请联系在线客服。";

        $text = "你好，您有即将逾期的商品，请尽快前往提货或使用！\n\n提货商品：[商品名称]\n到期时间：[核销时间] \n" . $remark;


        $message = array(
            'first' => array('value' => "你好，您有即将逾期的商品，请尽快前往提货或使用！", "color" => "#ff0000"),
            'keyword1' => array('title' => '提货商品', 'value' => $orders['title'], "color" => "#000000"),
            'keyword2' => array('title' => '到期时间', 'value' => $time, "color" => "#000000"),

            'remark' => array('value' => "\n请尽快处理，逾期将作废！如有任何疑问可与我们联系，感谢您的支持", "color" => "#000000")
        );

        $this->sendNotice(array(
            "openid" => $member['openid'],
            'tag' => 'willcancelverify',
            'default' => $message,
            'cusdefault' => $text,
            'url' => $url,
            'datas' => $datas
