...
class Order_EweiShopV2Model
{
    ...
    function fullback($orderid)
    {
        global $_W;
        $uniacid = $_W['uniacid'];
        $order_goods = pdo_fetchall("select o.openid,og.optionid,og.goodsid,og.price,og.total,og.id from " . tablename("ewei_shop_order_goods") . " as og
            ...
            if ($goods['isfullback'] > 0) {
                ...
                if (!empty($fullbackgoods)) {
                    ...
                    if (empty($has_record)) {
                        ...
                        $fullbackid = pdo_insertid();
                        pdo_update('ewei_shop_order_goods',array('fullbackid'=>$fullbackid),array('id'=>$value['id'],'uniacid'=>$_W['uniacid']));
                    }
                }
            }
            ...
            if ($params['type'] == 'cash') {
                ...
                if ($order['isparent'] == 1) {
                    ...
                    foreach ($merchData as $mk => $mv) {
                        //打印机打印
                        file_put_contents(EWEI_SHOPV2_DATA . 'print1.json',1111);
...
//                                }
    ...
    function payVirtualSend($orderid = 0, $ispeerpay = false)
    {
        ...
        $order = pdo_fetch('select id,uniacid,ordersn, price,openid,dispatchtype,addressid,carrier,status,isverify,deductcredit2,`virtual`,isvirtual,couponid,isvirtualsend,isparent,paytype,merchid,agentid,createtime,buyagainprice,istrade,tradestatus,iscycelbuy, wxapp_allow_subscribe from ' . tablename('ewei_shop_order') . ' where  id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
        ...
        if (!empty($order['wxapp_allow_subscribe'])) {
            ...
            if (in_array('autosend', $template)) {
                ...
                $msgdata['remark'] = '您的订单已发货';
                $msgdata['page'] = '/pages/order/detail/index?id=' . $orderid;
                if (p('app')) {
                    if (strexists('sns_wa', $order['openid'])) {
                        $openid = $order['openid'];
                    } else {
                        $openid = pdo_fetchcolumn("select openid_wa from " . tablename('ewei_shop_member') . " where openid='{$order['openid']}'");
                    }
    ...
    function setGiveBalance($orderid = '', $type = 0, $status = 0)
    {
        ...
        if ($balance > 0) {
            ...
            if ($type == 1) {
                ...
                // 已完成的订单扣除赠送金额 2018-08-27 zhurunfeng 修改
                //订单售后,扣除赠送的余额
                if ($order['status'] >= 1) {
...
//                    }
    ...
    function getGoodsDiscountPrice($g, $level, $type = 0, $memberCardId = 0)
    {
        ...
        if (p('membercard')) {
            ...
            // 此处判断不可去,因为会存在没有有会员卡插件但是没有会员卡的用户被误判,出现不享受折扣的问题
            if ($hasMemberCard) {
                // discount = 0 不开启折上折 1 - 开启折上折, 那么商品的促销和会员折扣是否开启也要跟设置一致
...
//                }
    ...
    {//查询是否是代付订单,如果不是返回false,如果是返回代付订单内容
        global $_W;
...
//        $url = 'http://newopen.qa.imdada.cn/api/order/addOrder';
            ...
            if ($cityexpress['express_type'] == 1) {
                ...
                if ($location_data['status'] == 1 && $location_data['count'] > 0) {
                    ...
                    $data = array(
                        ...
                        'cargo_weight'=>$order['weight']/1000,
                        'origin_id' => $order['ordersn'],//	第三方订单ID
                        'info' => $order['remark'],//订单备注
                        'cargo_price' => price_format($order['price']),//订单金额
                        ...
                        'callback' => 'http://newopen.imdada.cn/inner/cargo_weightapi/order/status/notify'
                    );
    ...
    function CheckoodsStock($orderid = '', $type = 0, $flg = true)
    {
        ...
        if (!$flg) {
            //如果是付款减库存的话在订单里边支付的时候会检测,就会导致重复几次就提示没库存了
            return true;
        }
