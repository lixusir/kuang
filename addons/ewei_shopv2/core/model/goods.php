...
class Goods_EweiShopV2Model {
    ...
    public function getList($args = array()) {
        ...
        if (!$random) {
            $sql = "SELECT id,title,subtitle,thumb,thumb_url{$officsql},marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sum(sales+salesreal) as salesreal,total,description,bargain,`type`,ispresell,isdiscount_time_start,isdiscount_time,`virtual`,hasoption,video,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay,presellprice,buylevels,buygroups
            ...
            $sql = "SELECT id,title,thumb,thumb_url{$officsql},marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sum(sales+salesreal) as salesreal,total,description,bargain,`type`,ispresell,isdiscount_time_start,isdiscount_time,`virtual`,hasoption,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay,presellprice,buylevels,buygroups
        ...
        foreach($list as $lk => $lv){
            ...
            if($lv['hasoption']==1) {
                ...
                $lv['marketprice'] = empty($pricemax) ? 0 : max($pricemax); // 如果导致商品价格显示错误 开启此块注释
            }
...
//                    秒杀
        ...
        where g.id = ".$lv['id']." and g.hasoption=1";
                ...
                pdo_update('ewei_shop_goods',array('minprice'=>$goodsinfo['minprice'],'maxprice'=>$goodsinfo['maxprice']),array('id'=>$lv['id'],'uniacid'=>$_W['uniacid']));
...
//            if((!empty($showlevels)&&in_array($member['level'], $showlevels)) || (!empty($showgroups) && in_array($member['groupid'], $showgroups)) || (empty($showlevels) && empty($showgroups))){
            if((!empty($showlevels)&&in_array($member['level'], $showlevels)) || (!empty($showgroups) && $isShow) || (empty($showlevels) && empty($showgroups))){  //判断会员组是否有查看和购买权限
                $showgoods = 1;
            }
        }else{
