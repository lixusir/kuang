...
//        error_reporting(E_ALL);
        ...
        if (intval($_GPC['ajax']) > 0 && intval($_GPC['loop']) > 0) {
            $dir_pre = intval($_GPC['dir_pre']);//获取前缀
            if ($redis && !is_error($redis)) {
                $t = $redis->get($dir_pre);
                if (!empty($t)) {
                    $dir_next = $t;
                } else {
                    $dir_next = $this->getRandChar(10);
                    $dir_next = md5($dir_next);//生成md5
                    $redis->set($dir_pre, $dir_next);
                }
            } else {
                $t = cache_load($dir_pre);
                if (!empty($t)) {
                    $dir_next = $t;
                } else {
                    $dir_next = $this->getRandChar(10);
                    $dir_next = md5($dir_next);//生成md5
                    cache_write($dir_pre, $dir_next);
                }
...
//            if ($_SESSION['dir_prev'] == $dir_pre){//如果session已有,说明已存在md5
//                $dir_next = $_SESSION['fileMD5'];//取出md5
//            }else{//如果没有session,不存在md5
//                $dir_next = $this->getRandChar(10);
//                $dir_next = md5($dir_next);//生成md5
//                $_SESSION['fileMD5'] = $dir_next;//md5存session
//                $_SESSION['dir_prev'] = $dir_pre;//存前缀
//            }
            $dir_pre .= $dir_next;//前缀拼合
            ...
            echo intval($_GPC['loop']) . '&' . $dir_pre;//jquery显示进度
                ...
                if ($res['code_type'] < 2) {
                    ...
                    if ($res['code_type'] == 2) {//普通二维码
                        ...
                        if (!file_exists("../addons/ewei_shopv2/data/qrcode/" . $_W['uniacid'] . "/exchange/" . $dir_pre . ".txt")) {
                            $text = fopen("../addons/ewei_shopv2/data/qrcode/" . $_W['uniacid'] . "/exchange/" . $dir_pre . ".txt", 'w');
//
                            $ret = fwrite($text, $key . '_' . $serial . $br);
                            fclose($text);
                        } else {
                            $text = fopen("../addons/ewei_shopv2/data/qrcode/" . $_W['uniacid'] . "/exchange/" . $dir_pre . ".txt", 'a+');
                            $ret = fwrite($text, $key . '_' . $serial . $br);
                        ...
                        if (intval($_GPC['end']) == 1) {
                            ...
                            $excel->getActiveSheet()->setCellValue('C1', '所属兑换活动');
                            $file = fopen("../addons/ewei_shopv2/data/qrcode/" . $_W['uniacid'] . "/exchange/" . $dir_pre . ".txt", 'r');
//                            echo "../addons/ewei_shopv2/data/qrcode/". $_W['uniacid'] ."/exchange/";exit();
                            $line = 2;
                            if (empty($file)) {
                                die();
                            }
    ...
    {//Ajax商品列表搜索
        ...
        $loop2 = count($option2);//tempalte中loop次数
        if (!empty($id)) {
            $setting = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_exchange_group') .
                " WHERE id=:id AND uniacid=:uniacid", array(':id' => $id, ':uniacid' => $_W['uniacid']));
            $goods = json_decode($setting['goods'], true);
    ...
    function addreply()
    {
        ...
        $url = mobileUrl('exchange.index', array('codetype' => 1, 'id' => $groupid), 1);//图文消息链接
        ...
        if (!empty($group['keyword_reply'])) {//update
