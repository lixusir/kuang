define(['core', 'tpl', 'swiper','./timer.js'], function (core, tpl,s,timerUtil) {
        ...
        if (status != -1) {
            $.ajax({
                ...
                complete: function (x) {
                    ...
                    if (status == 0) {
                        model.lasttime = endtime-currenttime;
                    } else {
                        model.lasttime = starttime-currenttime;
                    }
                    model.setTimer(timeid);
                    model.timer = model.setTimerInterval(timeid)
                }
            })
        }
    };
    model.setTimer = function (timeid) {
        //每十秒请求一次服务器，获取时间
        if(model.lasttime%10==0){
            var slide = $(".goods-slide[data-timeid='" + timeid + "']");
            var status = parseInt(slide.data('status')), starttime = slide.data('starttime'), endtime = slide.data('endtime');
            $.ajax({
    ...
    model.setTimerInterval = function (timeid) {
