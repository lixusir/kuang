...
class Index_EweiShopV2Page extends PluginMobilePage
{
    ...
    function main()
    {
        ...
        if (!empty($set['share'])) {
            $_W['shopshare'] = array(
                'title' => $set['enter_title'],
                'imgUrl' => tomedia($set['enter_img']),
                'desc' => $set['enter_desc'],
                'link' => mobileUrl('qa')
            );
            if (p('commission')) {
    ...
    function detail()
    {
        ...
        if (!empty($set['share'])) {
            $_W['shopshare'] = array(
                ...
                'link' => mobileUrl('qa/detail', array('id' => $item['id']), true)
            );
            if (p('commission')) {
                $commset = p('commission')->getSet();
                if (!empty($commset['level'])) {
    ...
    function question()
    {
        ...
        if (!empty($set['share'])) {
            $_W['shopshare'] = array(
                'title' => $category['name'],
                'imgUrl' => tomedia($category['thumb']),
                'desc' => m('plugin')->getName('qa'),
                'link' => mobileUrl('qa/question', array('cate' => $cate))
            );
            if (p('commission')) {
                $commset = p('commission')->getSet();
                if (!empty($commset['level'])) {
