...
class Single_EweiShopV2Page extends PluginWebPage {
    ...
    function getesheetTemp(){
        ...
        if($_W['ispost']){
            ...
            if ($esheet_item['code'] == 'JD'){
                $eorder["ExpType"] = 6;
            }
            //如果是顺丰陆运，type为2  徐子轩2020年3月25日11:43:59
            ...
            if($result["ResultCode"] == "100" || $result['Success'] === true) {
                ...
                if (empty($_GPC['is_group'])){
                    ...
                    foreach ($orderids as $orderid){
                        ...
                        //如果是拼团
                        $orderinfo = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_groups_order') . " WHERE id=:orderid and status>-1 and uniacid=:uniacid and merchid=0 limit 1", array(':orderid'=>$orderids[0],':uniacid' => $_W['uniacid']));
