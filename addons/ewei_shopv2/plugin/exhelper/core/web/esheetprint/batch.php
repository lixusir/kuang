...
class Batch_EweiShopV2Page extends PluginWebPage {
    ...
    function main(){
        ...
        //$isCLodop = $this->getLodop($lodopUrl);
        load()->func('tpl');
...
//			$condition .= " and ( o.addressid<>0  or ( o.addressid=0 and o.isverify=0 and o.virtual=0 and o.dispatchtype=1 and o.storeid=0) ) ";
        ...
        if($_W['ispost']){
            ...
            if($result["ResultCode"] == "100" || $result['Success'] === true) {
                ...
                if (empty($_GPC['is_group'])){
                    ...
                    //如果是拼团
                    $orderinfo = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_groups_order') . " WHERE id=:orderid and status>-1 and uniacid=:uniacid limit 1", array(':orderid'=>$orderids[0],':uniacid' => $_W['uniacid']));
                    ...
                    if($orderinfo['status']==1){
                        ...
                        if($esheet_item['issend']==1){
                            ...
                            if (!empty($orderinfo['refundid'])) {
                                ...
                                if (!empty($refund)) {
                                    pdo_update('ewei_shop_groups_order_refund', array('status' => -1, 'endtime'=>time()), array('id' => $orderinfo['refundid']));
                                    pdo_update('ewei_shop_groups_order', array('refundid' => 0), array('id' => $orderinfo['id']));
                                }
                            }
                        }
                        $update_res = pdo_update('ewei_shop_groups_order', $data, array('id' =>$orderinfo['id']));
                        if($update_res && $data['status']==2){
                            //日志
                            plog('exhelper.print.batch.dosend', "自动发货 订单号: {$orderinfo['orderno']} <br/>快递公司: {$_GPC['expresscom']} 快递单号: {$_GPC['expresssn']}");
                        }
                    }
                }
