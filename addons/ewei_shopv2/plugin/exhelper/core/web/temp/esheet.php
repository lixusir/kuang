...
class Esheet_EweiShopV2Page extends PluginWebPage {
	function main() {
        ...
        $sql = 'SELECT temp.id,temp.esheetname,esheet.name,temp.isdefault'
            .' FROM ' . tablename('ewei_shop_exhelper_esheet_temp') . ' temp '
            . ' left join ' . tablename('ewei_shop_exhelper_esheet') . ' esheet on temp.esheetid = esheet.id '
            . ' where 1 ' . $condition . ' ORDER BY temp.isdefault DESC, id LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
	...
	protected function post(){
        ...
        /*$data = array(
            ...
            2 => array(
                'style' => '二联150',
                'spec' => '(宽100mm 高150mm 切点90/60)',
                'size' => '150',
                'isdefault' => 0
            )
        );

        dump(serialize($data));die;*/
		...
		if($_W['ispost']){
			...
			$id =  intval($_GPC['id']);
			$data = array(
                'esheetid' => trim($_GPC['esheetid']),
				'esheetname' => trim($_GPC['esheetname']),
				'customername' => trim($_GPC['customername']),
				'customerpwd' => trim($_GPC['customerpwd']),
				'monthcode' => trim($_GPC['monthcode']),
				'sendsite' => trim($_GPC['sendsite']),
				'paytype' => intval($_GPC['paytype']),
				'templatesize' => trim($_GPC['templatesize'])==0?'':trim($_GPC['templatesize']),
				'isnotice' => intval($_GPC['isnotice']),
