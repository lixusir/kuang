	...
	function getdata(){
		...
		if($_W['ispost']){
			...
			foreach($orders as $i=>$order){
				...
				$order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,g.thumb,g.unit,g.goodssn,og.optionid,og.goodssn as option_goodssn, g.productsn, g.weight, og.productsn as option_productsn, og.total,og.price,og.optionname as optiontitle, og.realprice,og.printstate,og.printstate2,og.id as ordergoodid from ' . tablename('ewei_shop_order_goods') . ' og '
						. ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid '
						. ' where og.uniacid=:uniacid and og.merchid=0 and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
