...
class Index_EweiShopV2Page extends PluginMobilePage {
    ...
    public function getInfo(){
        ...
        //获取会员信息
        $openid = $_W['openid'];
        $where_tmp = $_GPC['merchid'] > 0 ? (" and merchid = " . intval($_GPC['merchid'])) : "";
...
//                    秒杀
                ...
                $goodsql = 'SELECT id,displayorder,title,subtitle,thumb,marketprice,productprice,minprice,maxprice,isdiscount,hascommission,nocommission,commission,commission1_rate,marketprice,commission1_pay,maxprice,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video,buylevels,buygroups FROM ' .tablename('ewei_shop_goods') .' WHERE id in('.$goodsid.') AND status > 0 AND deleted = 0 AND uniacid ='.$_W['uniacid']. $where_tmp .' limit 0,'.$goodsnum;
                $count = pdo_fetch('SELECT count(id) as count FROM ' .tablename('ewei_shop_goods') .' WHERE id in('.$goodsid.') AND status > 0 AND checked=0 AND deleted = 0 AND uniacid ='.$_W['uniacid'] . $where_tmp);
                $list['list'] = pdo_fetchall($goodsql);
                $list['count'] = $count['count'];
                if(!empty($list)){
...
//                    秒杀
                ...
                if(!empty($goodsids)){
                    ...
                    $sql = 'SELECT id,displayorder,title,subtitle,thumb,marketprice,productprice,minprice,maxprice,hascommission,nocommission,commission,commission1_rate,marketprice,commission,commission1_pay,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video,buylevels,buygroups,showlevels FROM ' .tablename('ewei_shop_goods') .' WHERE id in('.$goodsid.') AND uniacid ='.$_W['uniacid']. $where_tmp .' limit 0,'.$goodsnum;
                    $count = pdo_fetch('SELECT count(id) as count FROM ' .tablename('ewei_shop_goods') .' WHERE id in('.$goodsid.') AND checked=0 AND uniacid ='.$_W['uniacid'] . $where_tmp);
...
//                    秒杀
                            ...
                            if (!empty($member)) {
                                ...
                                //会员浏览权限
                                if ($value['showlevels'] != ''){
                                    $showlevels = explode(',', $value['showlevels']);
            ...
            if(!empty($storesids)){
                ...
                $sql = 'SELECT id,displayorder,storename FROM ' .tablename('ewei_shop_store') .' WHERE id in('.$storesid.') AND uniacid ='.$_W['uniacid']. $where_tmp . ' limit 0,'.$storenum;
                $count = pdo_fetch('SELECT count(id) as count FROM ' .tablename('ewei_shop_store') .' WHERE id in('.$storesid.') AND uniacid ='.$_W['uniacid'] . $where_tmp);
                $list['list'] = pdo_fetchall($sql);
                $list['count'] = $count['count'];
                if(!empty($list)){
                    m('common')->sortArrayByKey($list['list'], 'displayorder');
