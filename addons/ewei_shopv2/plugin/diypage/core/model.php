...
//                                    获取商品的预售价格
                        ...
                        if (!empty($content)) {
                            ...
                            //客户提出首页有富文本的时候加载特别慢影响体验(OA bug号:4875)
                            if ($page['type'] != 2) {
                                //首页富文本去掉懒加载
...
//获取分销商等级
    ...
    public function savePage($id, $data, $update = true)
    {
        ...
        if ( !is_mobile() && $update === true) {
            foreach ($data['items'] as &$item ) {
                if ($item['id'] == 'shortVideo') {
                    ...
                    if ($item['style']['upType'] == 1) {
                        ...
                        $item['params']['turl'] = $item['params']['shortOrignUrl'];
                    }
                } elseif ($item['id'] == 'detailVideo') {
                    ...
                    if ($item['style']['upType'] == 1) {
                        ...
                        $item['params']['turl'] = $item['params']['detailOrignUrl'];
                    }
                }
            }
        }
...
//        $str = preg_replace_callback("/\"linkurl\"\:\"?([^\'\" ]+).*?\"/", function($matches) use($mid){
    ...
    public function getStartAdv($id)
    {
        ...
        if ($startadv['params']['showtype'] > 0 && $startadv['params']['showtime'] > 0) {
            ...
            //修复 有的客户页面刷新两次 导致cookie设置两次导致不生效的.
