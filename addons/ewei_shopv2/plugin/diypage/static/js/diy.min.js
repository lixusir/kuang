define(['jquery.ui', '../../../../static/js/web/goods_selector.js','/../../../addons/ewei_shopv2/plugin/diypage/static/js/drag.js?V=' +  (new Date()).valueOf()], function (ui, gSelector) {
    var modal = {
        sysinfo: null,
        id: 0,
        type: 1,
    ...
    modal.initNavs = function () {
        modal.getNavs();
        var navgroup = {
            0: ['listmenu', 'richtext', 'title','video', 'line', 'blank', 'menu', 'menu2', 'picture', 'banner', 'picturew', 'pictures', 'icongroup', 'audio', 'coupon','tabbar', 'topmenu','hotarea'],
            1: ['search', 'fixedsearch', 'notice', 'goods', 'merchgroup', 'diymod', 'groupsgoods','shortVideo'],
            2: ['search', 'fixedsearch', 'notice', 'goods', 'merchgroup', 'seckillgroup', 'diymod', 'groupsgoods','shortVideo' ],
            3: ['member', 'bindmobile', 'logout', 'wxcard', 'verify','shortVideo'],
            4: ['memberc', 'commission_block', 'commission_sharecode', 'blockgroup','shortVideo'],
            5: ['detail_tab', 'detail_swipe', 'detail_info', 'detail_sale', 'detail_spec', 'detail_package', 'detail_shop', 'detail_store', 'detail_buyshow', 'detail_comment', 'detail_pullup', 'detail_navbar', 'detail_seckill', 'goods', 'groupsgoods','detailVideo'],
            6: ['goods', 'search', 'merchgroup','shortVideo'],
            7: ['seckill_times', 'seckill_rooms', 'seckill_advs', 'seckill_list','shortVideo'],
            8: ['exchange_banner', 'exchange_input', 'exchange_rule','shortVideo'],
            9: ['tabbar','shortVideo']
        };
    ...
    modal.getNavs = function () {
        modal.navs = {
            hotarea: {
                name: '热区',
                params: {
                    bgimg: '../addons/ewei_shopv2/plugin/diypage/static/images/default/cube-1.jpg',
                },
                style: {
                    'paddingtop': '10',
                    'paddingleft': '10',
    ...
    modal.initEditor = function (scroll) {
        ...
        // H5添加热区并初始化
        var hotareaModalLength = $("#diy-editor .hotareaModal").length;
        if(hotareaModalLength > 0 ) {
            // TODO 初始化添加拖拽与缩放事件
            $('#diy-editor .hotareaModal .draggable').each(function (i,item) {
                bindEvents($(item));
            })
            // TODO .hotsave的点击事件
            if(modal.timesHot==0) {
            $(document).on('click','.hotsave',function () {
                modal.timesHot++;
                $(this).parents('.hotareaModal').siblings('.item-image').find('.bigHot').remove();
                var drags = $('.draggable');
                var haveUrl = true;
                $.each(drags,function (i,item) {
                    if(!$(item).data('href')){
                        tip.msgbox.err('请选择链接地址');
                        $(drags[i]).css('background','rgba(255, 31, 44, 0.5)').css('border', '1px solid #ff1f2c');
                        return haveUrl = false;
                    }
                })
                if(haveUrl) {
                    var postData = {}, hots = $('.draggable'),len = hots.length, bigW = parseInt($('.hotContainer').width()), bigH = parseInt($('.hotContainer').height());
                    var itemid = modal.selected;
                    for (var i=0; i<len; i++) {
                        var childid = modal.getId('C', i);
                        var numW = ($(hots[i]).width() / bigW ) *100 + '',
                            numH = ($(hots[i]).height() / bigH ) *100 + '',
                            numT = (parseInt($(hots[i]).css('top'))/bigH)*100+'',
                            numL = (parseInt($(hots[i]).css('left')) / bigW) *100 + '';
                        var item = {
                            width : Math.round(numW* 100) / 100.00 + '%' ,
                            height : Math.round(numH * 100) / 100.00 + '%' ,
                            top : Math.round(numT * 100) / 100.00 + '%' ,
                            left : Math.round(numL* 100) / 100.00 + '%' ,
                            href: $(hots[i]).data('href'),
                            title: $(hots[i]).find('.hotTitle').text(),
                            zIndex: $(hots[i]).css('z-index')
                        }
                        postData[childid] = item;
                    }
                    modal.items[itemid].data = postData;
    ...
    var close =0;
    // 热区图片监听点击事件
    $(document).on('click','.hotClose',function () {
        close++;
        $(this).parent().parent().parent().hide();
    });

    $(document).on('click','.delHot',function(){
        var that = $(this)
        tip.confirm('确认要删除此热区吗', function () {
            $(that).parents('.draggable').remove();
        },function () {
            return false;
        });
    })
    // 热区添加
    $(document).on('click','.addHotarea',function() {
        var src = $('#form-items .hotImg').attr('src'),
            parents = $(this).parent(),
            curl = $(this).data('url');
        $('.hotBody .min-box .hotContainer img').attr('src',src);
        if(close >0) {
            if($(this).parent().hasClass('item-image') || $(this).hasClass('btn-w-m') ) {
                var cont = $(this).parents('.inner ').find('.bigHot'),
                    bigW = $(this).parents('.inner ').find('img').width(),
                    bigH = $(this).parents('.inner ').find('img').height();
                $(this).parent().siblings('.hotareaModal').find('.hotContainer .draggable').remove();
                for (var i=0;i<cont.length;i++) {
                    var numW = ($(cont[i]).width() / bigW ) *100 + '',
                        numH = ($(cont[i]).height() / bigH ) *100 + '',
                        numT = (parseInt($(cont[i]).css('top'))/bigH)*100+'',
                        numL = (parseInt($(cont[i]).css('left')) / bigW) *100 + '';
                    var w = Math.round(numW* 100) / 100.00 + '%' ,
                        h = Math.round(numH * 100) / 100.00 + '%' ,
                        t = Math.round(numT * 100) / 100.00 + '%' ,
                        l = Math.round(numL* 100) / 100.00 + '%' ,
                        href= $(cont[i]).data('href'),
                        text = $(cont[i]).text(),
                        zIndex= $(cont[i]).css('zIndex');
                    var obj= $('<div class="draggable dragable-item" data-ischeck="false" style="z-index:' + zIndex+'; position:absolute;top: ' + t+';left:' + l+'; user-select: auto;width:' + w+';height: ' + h+';" data-href="' + href+'">' +
                        '<div class="hotTitle">' + text+'</div>'
                        +'<div class="handle rLeftUp" style="left: -5px;top: -5px;"></div>'
                        +'<div class="handle rUp" style="left: calc(50% - 4px);top: -5px;"></div>'
                        +'<div class="handle rRightUp" style="right: -5px;top: -5px;"></div>'
                        +'<div class="handle rRight" style="right: -5px;top: calc(50% - 4px);"></div>'
                        +'<div class="handle rRightDown" style="right: -5px;bottom: -5px;"></div>'
                        +'<div class="handle rDown" style="right: calc(50% - 4px);bottom: -5px;"></div>'
                        +'<div class="handle rLeftDown" style="bottom: -5px;left: -5px;"></div>'
                        +'<div class="handle rLeft"  style="left: -5px;top: calc(50% - 4px);"></div>'
                        +'<div class="addUrl" data-toggle="selectUrl" data-input="'+curl+'">选择链接</div>'
                        +'<span class="delHot" >×</span></div>');
                    $(this).parent().siblings('.hotareaModal').find('.hotContainer').append(obj);
                    bindEvents(obj);
                }
            }
        }
        var index = $('#hot1 .draggable').length+1;
        var obj = $('<div class="draggable dragable-item" data-ischeck="false" style="z-index:' + index+'; position:absolute;top: 10px;left: 10px; user-select: auto;width: 100px;height: 100px;">' +
            ...
            +'<div class="addUrl" data-toggle="selectUrl" data-input="'+curl+'">选择链接</div>'
            +'<span class="delHot" >×</span></div>');
        if($(this).parent().parent().hasClass('hotfooter')) {
            $('#hot1').append(obj);
        }else {
            if($('#hot1 .draggable').length > 0) {
                $('.hotareaModal').show();
                return
            }else {
                $('#hot1').append(obj);
    ...
    $(document).on('click','.draggable',function () {
        $(this).css('background', 'rgba(45, 140, 240, 0.6)').css('border','1px solid #2d8cf0');
    })
    $(document).on('click','.btn.btn-default.btn-sm',function () {
        that = $(this);

        var tmp_url = that.data('href');
        if (tmp_status == 1)
        {
            var tmp_title;
            var tabs = $('#selectUrlTab .active a').attr('href');
            tabs = tabs.replace('#sut_','');
            if(tabs == 'shop') {
                tmp_title = that.attr('title');
            }
            else if(tabs == 'good'){  // 商品
                tmp_title = that.siblings('.text').find('.name').text();
            }
            else if(tabs == 'article' || tabs == 'coupon' || tabs == 'diypage' || tabs == 'groups' || tabs == 'sns' || tabs == 'quick' || tabs == 'cate'){  // 文章营销
                tmp_title = that.siblings('.text').text();
            }
            else if(tabs == 'creditshop'){  // 积分商城
                tmp_title = that.siblings('.text').find('.name').text();
            }
            //赋值
            tmp_obj.siblings('.hotTitle').text(tmp_title);
            tmp_obj.parent().attr('data-href',tmp_url);
            tmp_obj.attr('data-text',tmp_title);
            return false;
        }
        tmp_url = '';
        tmp_title = '';
