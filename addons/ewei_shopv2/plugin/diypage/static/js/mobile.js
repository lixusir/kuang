define(['core', 'tpl'], function (core, tpl) {
    ...
    modal.initTabbar = function(notskip,url,$this,num){
        if(notskip == 1 && url != ''){
            var merchid = $('#_merchid').eq(0).val();
            if(url.indexOf('stores') >= 0){
                $.get(core.getUrl('diypage/getInfo'),{url:url,num:num,paramsType:'stores',merchid:merchid},function(ret){
                    var ret = JSON.parse(ret);
                    if(ret.status == 1){
                        var list = ret.result.list;
                        var html = '';
                ...
                $.get(core.getUrl('diypage/getInfo'),{url:url,num:num,paramsType:'goods',merchid:merchid},function(ret){
                    var ret = JSON.parse(ret);
                    if(ret.status == 1){
                        var list = ret.result.list;
                        var html = '';
