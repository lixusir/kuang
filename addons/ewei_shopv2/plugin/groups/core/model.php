...
//            $teamnum = intval($order_goods['teamnum']) + 1;
    ...
    public function sendTeamMessage($orderid = '0', $delRefund = false)
    {
        ...
        // 拼团成功时间
        $successTime = pdo_fetchcolumn("select paytime from " . tablename('ewei_shop_groups_order') . " where uniacid=:uniacid and teamid=:teamid order by paytime desc", array(
            ':uniacid'  => $uniacid,
            ':teamid'   => $order['teamid']
        ));
    ...
    public function allow($orderid, $times = 0,$verifycode = '',$openid = '') {
        ...
        $order_goods = pdo_fetch("select id,option_name from " . tablename('ewei_shop_groups_order_goods') . "
			where uniacid=:uniacid and groups_goods_id = :groups_goods_id and groups_order_id = :groups_order_id",
            array(':uniacid' => $uniacid, ':groups_goods_id' => $goods['id'],':groups_order_id'=>$orderid));
