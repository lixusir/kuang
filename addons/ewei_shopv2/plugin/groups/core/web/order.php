...
class Order_EweiShopV2Page extends PluginWebPage {
    function main() {
        ...
        if ($_GPC['export'] == 1) {
            ...
            $columns = array(
                ...
                array('title' => '支付金额', 'field' => 'price', 'width' => 12),
                array('title' => '折扣金额', 'field' => 'discount', 'width' => 12),
    ...
    function refund($tid, $fee = 0, $reason = ''){
        ...
        if ($_W['ispost']) {
            ...
            //扣除会员购物积分
            /*if($credits>0){
                m('member')->setCredit($item['openid'], 'credit1', -$credits, array(0, $shopset['name'] . "退款扣除积分: {$credits} 订单号: " . $item['orderno']));
            }*/
    ...
    function closeOrder()
    {
        ...
        $set = pdo_fetch("SELECT * FROM ".tablename('ewei_shop_groups_set')." WHERE uniacid = :uniacid ",array(':uniacid'=>$_W['uniacid']));

        $shopset = $_S['shop'];



        $orders = pdo_fetch("select id,orderno,refundid,openid,credit,creditmoney,price,freight,status,pay_type,teamid,apppay,isborrow,borrowopenid,more_spec from " . tablename('ewei_shop_groups_order') . "
            where  uniacid={$_W['uniacid']} and status = 1 and pay_type !='other' and refundtime = 0 and id={$_GPC['id']}");


        $realprice = $orders['price'] - $orders['creditmoney'] + $orders['freight'];

        $credits = $orders['credit'];


        if ($orders['pay_type'] == 'credit') {

            //余额支付，直接返回余额
            $result = m('member')->setCredit($orders['openid'], 'credit2', $realprice, array(0,  "拼团退款: {$realprice}元 订单号: " . $orders['orderno']));

        } else if ($orders['pay_type'] == 'wechat') {
            //微信支付，走退款 接口
            //直接退还扣除减掉余额抵扣
            $realprice = round($realprice, 2);
            if (empty($orders['isborrow'])){
                $result = m('finance')->refund($orders['openid'], $orders['orderno'], $orders['orderno'], $realprice * 100, $realprice * 100, !empty($orders['apppay']) ? true : false);
            }else{
                $result = m('finance')->refundBorrow($orders['borrowopenid'], $orders['orderno'], $orders['orderno'], $realprice * 100, $realprice * 100, !empty($orders['apppay']) ? true : false);
            }

            $refundtype = 2;
        } elseif( $orders['pay_type'] == 'wxapp' ) {
            //微信小程序支付，走小程序退款 接口
            //直接退还扣除减掉余额抵扣
            $realprice = round($realprice, 2);
            $result = m('finance')->wxapp_refund($orders['openid'], $orders['orderno'], $orders['orderno'], $realprice * 100, $realprice * 100);
            ...
            $result = m('finance')->pay($orders['openid'], 1, $realprice * 100, $orders['orderno'], "退拼团款: {$realprice}元 订单号: " . $orders['orderno']);
            $refundtype = 1;
        }


        if ($credits > 0) {
            m('member')->setCredit($orders['openid'], 'credit1', $credits, array('0', "拼团购物返还抵扣积分 积分: {$orders['credit']} 抵扣金额: {$orders['creditmoney']} 订单号: {$orders['orderno']}"));
        }


        $refund = pdo_fetch('select * from ' . tablename('ewei_shop_groups_order_refund') . ' where id=:id limit 1', array(':id' => $orders['refundid']));
        if(empty($refund)!=true){
            if($refund['refundstatus']==0){
                ...
                //同意
                pdo_update('ewei_shop_groups_order_refund', $change_refund, array('id' => $orders['refundid']));
            }
        }
        //更新订单退款状态
        pdo_update('ewei_shop_groups_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => time()), array('id' => $orders['id'], 'uniacid' => $_W['uniacid']));

        //更新实际销量
        $sales = pdo_fetch("select id,sales,stock from ".tablename('ewei_shop_groups_goods')." where id = :id and uniacid = :uniacid ",array(':id'=>$orders['goodid'], ':uniacid' => $_W['uniacid']));

        pdo_update('ewei_shop_groups_goods', array('sales' => $sales['sales'] - 1,'stock'=>$sales['stock'] + 1), array('id' => $sales['id'], 'uniacid' => $_W['uniacid']));
        //如果是多规格
        if( $orders['more_spec'] == 1 ){
            $option = pdo_get( 'ewei_shop_groups_order_goods' , array( 'uniacid' => $_W['uniacid'] , 'groups_order_id' => $orders['id'] ) );
            //更新多规格库存
            pdo_update( 'ewei_shop_groups_goods_option' , array( 'stock' => 'stock+1' ),array( 'id' => $option['groups_goods_option_id'] ) );
        }

        plog('groups.task.refund', "订单退款 ID: {$orders['id']} 订单号: {$orders['orderno']}");
