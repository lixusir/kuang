...
class Order_EweiShopV2Page extends WebPage {
    ...
    protected  function orderData($status, $st){
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.agentid>0' || !empty($sqlcondition)){
            ...
            $sql = "select o.*  from " . tablename('ewei_shop_order') . " o"
                . " $sqlcondition where $condition $statuscondition GROUP BY o.id ORDER BY o.createtime DESC  ";
            ...
            if (!empty($list)){
                ...
                $verifyopenid_merch_array = pdo_fetchall("SELECT sm.id as salerid,sm.nickname as salernickname,sm.openid,s.salername FROM " .tablename('ewei_shop_merch_saler')." s LEFT JOIN ".tablename('ewei_shop_member')." sm ON sm.openid = s.openid and sm.uniacid=s.uniacid WHERE s.openid IN ({$verifyopenid})",array(),'openid');
                foreach ($list as $key=>&$value){
                    ...
                    if ($value['merchid'] > 0){
                        $list[$key]['salerid'] = $verifyopenid_merch_array[$value['verifyopenid']]['salerid'];
                        $list[$key]['salernickname'] = $verifyopenid_merch_array[$value['verifyopenid']]['salernickname'];
                        $list[$key]['salername'] = $verifyopenid_merch_array[$value['verifyopenid']]['salername'];
                    }else{
