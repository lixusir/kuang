...
class Apply_EweiShopV2Page extends PluginWebPage {
    ...
    function main() {
        ...
        $agentlevels = array_merge(array($default),$agentlevels);
        ...
        if (!empty($_GPC['agentlevel'])) {
            if ($_GPC['agentlevel'] == 'default'){
                $condition.=' and m.agentlevel=0';
            }else{
        ...
        $sql = "select a.*, m.nickname,m.avatar,m.realname,m.mobile,m.agentlevel,l.levelname,l.levelname,a.realname as applyrealname,a.sendmoney from " . tablename('ewei_shop_commission_apply') . " a "
        ...
        if ($status == 3){
            $realmoney_total = (float)pdo_fetchcolumn("select sum(a.realmoney) from " . tablename('ewei_shop_commission_apply') . " a "
                . " left join " . tablename('ewei_shop_member') . " m on m.id = a.mid"
                . " left join " . tablename('ewei_shop_commission_level') . " l on l.id = m.agentlevel"
        ...
        if ($_GPC['export'] == '1') {
            ...
            m('excel')->export($exportlist, array(
                "title" => $applytitle."佣金申请数据-" . date('Y-m-d-H-i', time()),
                "columns" => $columns
            ));
        }
...
//            dump($goods);exit;
    ...
    function check_ajax() {
        ...
        foreach ($ogids as $ogid) {
            $g = pdo_fetch("SELECT total, orderid,commission1,commission2,commission3,commissions from " . tablename('ewei_shop_order_goods') . "  "
                . "where id=:id and uniacid = :uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':id' => $ogid));
            if (empty($g)) {
                continue;
            }
            ...
            }else  if($order_level[$g['orderid']]==3){
                if($ischeck){
...
//        $pay = $totalpay;
        ...
        if ($apply['type'] < 2) {
            ...
            if (!empty($data['paytype']['commission']) && $apply['type']== 1){
                if(strpos($member['openid'],'sns_wa_')===false){
                    ...
                    pdo_update('ewei_shop_commission_apply',array(
            ...
            if (!empty($sec['alipay_pay']['open'])){
                ...
                if($sec['alipay_pay']['sign_type'] == 1){
                    $single_res = m('finance')->singleAliPay(array(
                        ...
                        'name'=>$apply['realname'],
                        'money'=>$pay
                    ),$batch_no,$sec['alipay_pay'], $set['texts']['commission'].'打款');
        ...
        //股东升级
        if(p('globonus')){
            p('globonus')->upgradeLevelByCommissionOK($member['openid']);
        }
