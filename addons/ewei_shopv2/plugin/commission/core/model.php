    ...
    class CommissionModel extends PluginModel
    {
        ...
        public function getSet($uniacid = 0)
        {
            ...
            $set['texts'] = array(
                ...
                'orderlevel' => empty($set['texts']['orderlevel']) ? '分销等级' : $set['texts']['orderlevel'],
                'moneytext' => $moneytext,
            );
            return $set;
        }
        ...
        public function checkAgent($openid = '')
        {
            ...
            if (empty($set['become'])) { //无条件
                ...
                $goodscount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_order_goods') . ' og '
                    . '  left join ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid'
                    . " where og.goodsid IN (".implode(',',$set['become_goodsid']).") and o.openid=:openid and o.status>={$order_status}  limit 1", array(':openid' => $openid));
                    ...
                    foreach ($res as $key => $value) {
                        if ($key == 0) {
                            ...
                            if ($tag == 'commission_order_pay') {
                                $text =  $data['nickname'] . '已付款！';
                            } else if ($tag == 'commission_order_finish') {
                                $text = $data['nickname'] . '已确认收货！';
                            }
                        }
                        if ($key == 1) {
                            foreach ($datas as $k => $v) {
