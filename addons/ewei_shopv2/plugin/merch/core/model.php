...
if (!pdo_tableexists('ewei_shop_domain_bindings')) {
    pdo_query("CREATE TABLE " . tablename('ewei_shop_domain_bindings') . " (
...
if (!is_file(IA_ROOT . '/web/merchant.php') || empty(cache_load('is_new_change')))
{
    copy(EWEI_SHOPV2_PLUGIN . 'merch/core/inc/merchant.php',IA_ROOT . '/web/merchant.php');
    cache_write('is_new_change',1);
}
...
FROM 
	...
	AND o.id = og.orderid where o.uniacid = :uniacid and o.merchid = :merchid  and refund.operatetime<:operatetime and o.status = 3 and o.finishtime < :finishtime and o.merchapply<=0 ";
        ...
        //计算完真实价格之后 把售后的订单金额加上
        $list['orderprice'] = $list['price'] + $refundorderprice;
        if ($deduct_commission) {
            $list['realprice'] -= $list['commission'];
        }
        $list['realpricerate'] = (100 - floatval($list['payrate'])) * $list['price'] / 100;
        if ($deduct_commission) {
        ...
        $list['orderids'] = array_unique($list['orderids']);
        return $list;
    }
    ...
    public function getMerchPriceList($merchid, $orderid = 0, $flag = 0, $applyid = 0, $pindex = 0, $psize = 0)
    {
        ...
        foreach ($order as &$list) {
            ...
            if ($deduct_commission) {
                $list['orderprice'] -= $list['commission'];
            }
            //可申请提现金额
    ...
    public function getMerchCreditList($merchid, $orderid = 0, $flag = 0, $creditrate = 0, $isbillcredit = 1)
    {
        ...
        if (!empty($_order)) {
            foreach ($_order as &$list) {
                ...
                //可申请提现金额
                $list['realprice'] = $list['realcreaterate'] + $list['money'] + $list['dispatch'];//提现金额
...
//订单金额
    ...
    public function getPluginList($merchid = 0)
    {
        ...
        if (p('exhelper') && $perm && $perm->is_perm_plugin('goodshelper')) {
            $has_plugins[] = 'goodshelper';
        }
        ...
        /*
        if (p('mmanage')&& $perm->is_perm_plugin('mmanage')) {
            $has_plugins[] = 'mmanage';
        }*/
