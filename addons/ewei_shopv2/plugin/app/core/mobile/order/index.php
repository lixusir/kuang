...
//            print_r($row['goods']);exit;
    ...
    function detail()
    {
        ...
        if (!empty($order['invoicename']) && !empty($order['invoice_img']))
        {
            $order['invoicename']['image'] = tomedia($order['invoice_img']);
        }
       ...
       foreach($gift as $kek=>$val){
        ...
        $card_free_dispatch = false;        //会员卡包邮
        ...
        if($plugin_membercard){
            ...
            if($ifuse){
                ...
                $membercard_info['discount_rate'] = $ifuse['discount_rate'];    //折扣
                if($ifuse['shipping']) $card_free_dispatch = true;
                $membercard_info['card_free_dispatch'] = $card_free_dispatch;   //包邮
            }
        }
    ...
    function express()
    {
        ...
        if(!empty($cycelid)){
            ...
            if ($order['status'] < 2 && $order['sendtype'] == 0) {
                return app_error(AppError::$OrderNoExpress);
            }
        }
    ...
    function cycelbuy_list(){
        ...
        $status0=0;//未开始数
        $status2=0;//已完成数
        $period_index=1;//当前第几期，默认取1
        $receipttime=$cycelData[0]['receipttime'];//当期收货时间，默认取第一期
...
//        }
        ...
        if(!empty($_GPC['isall'])){
            ...
            $new_data['cycleid'] = $_GPC['cycelid'];  //每期id
            $new_data['isall'] = 0;
        }
