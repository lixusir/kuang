...
class Favorite_EweiShopV2Page extends AppMobilePage {
 ...
 FROM ' . tablename('ewei_shop_member_favorite') . ' f '
            ...
            if (!empty($list) && $merch_plugin && $merch_data['is_openmerch'])
            {
                ...
                {
                    if (empty($row['title'])){
                        $row['title'] = $row['favtitle'];
                        $row['delete'] = 1;
                    }
                    if (empty($row['thumb'])){
                        $row['thumb'] = $row['favthumb'];
                    }
                    if (empty($row['marketprice'])){
                        $row['marketprice'] = $row['favmarketprice'];
                    }
                    if (empty($item['productprice'])){
                        $row['productprice'] = $row['favproductprice'];
                    }
    ...
    function get_merchlist(){
        ...
        $list = array();
        if (!empty($total)) {
            $sql = 'SELECT f.id,f.merchid,g.merchname,g.logo,g.desc FROM ' . tablename('ewei_shop_member_favorite') . ' f '
                . ' left join ' . tablename('ewei_shop_merch_user') . ' g on f.merchid = g.id '
                . ' where 1 ' . $condition . ' ORDER BY `id` DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
            $list = pdo_fetchall($sql, $params);
