...
class Order_EweiShopV2Page extends AppMobileAuthPage{
    ...
    public function main(){
        return app_json(array(
            'perm' => array(
                ...
                'order2' => cv('order.list.status2'),
                'order3' => cv('order.list.status3'),
                'order_1' => cv('order.list.status_1'),
                'order4' => cv('order.list.status4'),
                'order5' => cv('order.list.status5'),
                'refund' => cv('order.op.refund')
            )
        ));
    }
    ...
    public function get_list() {
        ...
        return app_json(array(
            ...
            'pagesize'=>$psize,
            'perm'=>array(
                'order_pay'=>cv('order.op.pay'),
                'order_send'=>cv('order.op.send'),
                'order_verify'=>cv('order.op.verify'),
                'order_fetch'=>cv('order.op.fetch'),
                'order_finish'=>cv('order.op.finish'),
                'order_sendcancel'=>cv('order.op.sendcancel'),
                'order_remarksaler'=>cv('order.op.remarksaler')
            )
        ));
    }
    ...
    public function send() {
        ...
        if($item['sendtype']>0){
            //未发货商品
            $noshipped = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('ewei_shop_order_goods') . ' og '
                . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid '
            ...
            for($i=1;$i<=$item['sendtype'];$i++){
                $row = array(
                    'sendtype'=>$i,
                    'goods'=>pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('ewei_shop_order_goods') . ' og '
                        . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid '
        ...
        $order_goods = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('ewei_shop_order_goods') . ' og '
            . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid '
        ...
        return app_json(array(
            ...
            'order_goods'=>$order_goods,
            'noshipped'=>set_medias($noshipped, 'thumb'),
            'shipped'=>$shipped
        ));
    }
    ...
    public function sendcancel() {
        ...
        $bundles = array();
        if($sendtype>0){
            $sendgoods = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('ewei_shop_order_goods') . ' og '
                . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid '
            ...
            if($item['sendtype']>0){
                for($i=1;$i<=intval($item['sendtype']);$i++){
                    $bundles[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('ewei_shop_order_goods') . ' og '
                        . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid '
    ...
    public function changeexpress() {
        ...
        if($sendtype>0){
            ...
            if($item['sendtype']>0){
                for($i=1;$i<=intval($item['sendtype']);$i++){
                    $bundle = array(
                        'goods' => pdo_fetchall('select g.id,g.title,og.optionname,og.total,g.thumb,og.sendtype,g.ispresell from ' . tablename('ewei_shop_order_goods') . ' og '
                            . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid '
                            . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype='.$i.' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id'])),
                        'sendtype'=>$i
                    );
                    ...
                    /*
                    $bundles[$i]['goods'] = pdo_fetchall('select g.id,g.title,og.optionname,og.total,g.thumb,og.sendtype,g.ispresell from ' . tablename('ewei_shop_order_goods') . ' og '
                        . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid '
                        . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype='.$i.' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
                    $bundles[$i]['sendtype'] = $i;
                    if(empty($bundles[$i]['goods'])){
                        ...
                        $bundles[$i]['goods'] = set_medias($bundles[$i]['goods'], 'thumb');
                    }*/
                }
            }
        }
