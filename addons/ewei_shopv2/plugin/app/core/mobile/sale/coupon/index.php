...
class Index_EweiShopV2Page extends AppMobilePage
{
    ...
    public function pay($a = array(), $b = array()) {
        ...
        if($goodsdetail){
            ...
            if ($coupon['credit'] > 0) {
                ...
                if(intval($coupon['credit'])>$credit){
                    return app_error(AppError::$CouponCanNotBuy, "您的积分不足，无法{$coupon['gettypestr']}!"); //已经领取完
                }
            }
            $needpay = false;
            if ($coupon['money'] > 0) {
        ...
        if($goodsdetail){
            ...
            if ($needpay) {
                $useweixin = true;  //需额外支付
                if (!empty($coupon['usecredit2'])) {
                    $money = $this->member['credit2']; //优先使用余额支付
                    if ($money >= $coupon['money']) {   //如果足够
                        $useweixin = false;
                    }
                }
                //支付方式
