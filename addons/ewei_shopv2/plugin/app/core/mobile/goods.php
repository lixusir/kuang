...
//        }
        ...
        if($goods['isdiscount'] && $goods['isdiscount_time']>=time() && $goods['isdiscount_time_start'] < time()){
            ...
            if ($goods['hasoption']){
                $max = pdo_fetch(" SELECT marketprice FROM ".tablename('ewei_shop_goods_option'). " WHERE uniacid=:uniacid AND goodsid=:goodsid ORDER BY marketprice DESC ",array(':uniacid'=>$_W['uniacid'],':goodsid'=>$goods['id']));
                $min = pdo_fetch(" SELECT marketprice FROM ".tablename('ewei_shop_goods_option'). " WHERE uniacid=:uniacid AND goodsid=:goodsid ORDER BY marketprice ASC ",array(':uniacid'=>$_W['uniacid'],':goodsid'=>$goods['id']));
...
//                    秒杀
                ...
                if ($member['agenttime'] > 0 && $member['isagent'] == 1)
                {
...
//            $minprice = $maxprice = $goods['marketprice'] = $seckillinfo['price'];  //秒杀价格显示问题 lgt
        ...
        /**
         * @var $touch_loop Touch_loopModel
         */
        $touch_loop = p('touch_loop');
        if ($touch_loop) {
            $set = $touch_loop->getSet();
            $info = pdo_get('ewei_shop_touch_loop_list', array('goods_id' => $goods['id'], 'uniacid' => $uniacid, 'status' => 1));
            if (!empty($info)) {
                $info['thumbs'] = unserialize($info['thumbs']);

                set_medias($info, 'thumbs');
                foreach ($info['thumbs'] as &$tmp_info) {
                    $tmp_info = tomedia($tmp_info);
                }
                unset($tmp_info);
                $goods['tbs'] =  $info['thumbs'];

                $goods['tbs_text'] = $info['content'];
            } else {
                if ($set['type'] == 1) {
                    $goods['tbs'] = $goods['thumbs'];
                    $goods['tbs_text'] =$goodsdesc;
                }
            }
            $goods['icons'] = tomedia($set['icon']);
        }
        $goods['video_cut'] = tomedia($goods['video_cut']);
    ...
    public function get_picker() {
        ...
        if( $seckill){
            ...
            foreach ($seckillinfo['options'] as &$op)
            {
                $option_tmp = pdo_get('ewei_shop_goods_option',array('id'=>$op['optionid'],'goodsid'=>$op['goodsid'],'uniacid'=>$op['uniacid']));
//                echo json_encode(end(pdo_debug(false)));
//                exit();
                $op['specs'] = $option_tmp['specs'];
            }
            unset($op);
            if(!empty($seckillinfo)){
                if($time >= $seckillinfo['starttime'] && $time<$seckillinfo['endtime']){
        ...
        if($goods['isdiscount'] && $goods['isdiscount_time'] >= time() && $goods['isdiscount_time_start'] < time()){
            ...
            if (!isset($isdiscount_discounts['type']) || empty($isdiscount_discounts['type'])) {
                ...
                //详细促销
                $level = m('member')->getLevel($openid);
                $goods_discounts = m('order')->getGoodsDiscounts($goods, $isdiscount_discounts, $level['id'], $options);
            ...
            $minprice = min($prices);
            $maxprice = max($prices);
            $goods['minprice'] = (float)$minprice;
            $goods['maxprice'] = (float)$maxprice;
        }
                ...
                if(!isset($discounts['type']) || empty($discounts['type'])) {
                    ...
                    if(!empty($discounts[$key])) {
                        ...
                        if(!empty($dd)) {
                            ...
                            $memberprice_dis = round($md / 10 * $minprice, 2);// XZX2019.9.21
