...
if(!class_exists('AppModel')) {
    ...
    class AppModel extends PluginModel {
        ...
        public function wxpay($params, $type = 0){
            ...
            $package['attach'] = $_W['uniacid'] . ':' . $type  .':1' ;
        ...
        public function getAccessToken(){
            ...
            if(!empty($accessToken) && !empty($accessToken['token']) && $accessToken['expire'] > TIMESTAMP) {
        ...
        public function sendNotice($openid = null, $datas = array(), $prepay_id = null, $orderid = 0, $type = 'pay'){
            ...
            foreach ($tempate['datas'] as $index => $item) {
                ...
                $data[$key] = array(
                    'value' => $this->replaceTemplate($item['value'], $datas),
                    'color' => $item['color']
                );
            ...
            $obj = json_encode(array(
                ...
                'form_id' => $prepay_id,
                'data' => $data,
                'emphasis_keyword' => $emphasis_keyword . '.DATA'
            ));
        ...
        public function getTMessage($id){
            ...
            $item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_wxapp_tmessage') . ' WHERE id=:id AND uniacid=:uniacid AND status=1 LIMIT 1', array(
                ':id' => $id,
                ':uniacid' => $_W['uniacid']
            ));
                            ...
                            if($item['id'] == 'richtext') {
                                ...
                                if(!empty($notices) && is_array($notices)) {
                                    foreach ($notices as $index => $notice) {
                                        ...
                                        $item['data'][$childid] = array(
                                            'id' => $notice['id'],
                                            'title' => $notice['title'],
                                            'linkurl' => '/pages/shop/notice/detail/detail?id=' . $notice['id']
                                        );
                                    }
                                }
                                ...
                                if(!empty($item['data'])) {
                                    ...
                                    if($item['params']['showicon'] == 1) {
                                        ...
                                        } elseif($item['style']['goodsicon'] == 'hotsale') {
                                            $item['style']['goodsicon'] = '热销';
                                        } elseif($item['style']['goodsicon'] == 'isnew') {
                                            $item['style']['goodsicon'] = '新上';
                                        } elseif($item['style']['goodsicon'] == 'sendfree') {
                                            $item['style']['goodsicon'] = '包邮';
                                        } elseif($item['style']['goodsicon'] == 'istime') {
                                            $item['style']['goodsicon'] = '限时购';
                                        } elseif($item['style']['goodsicon'] == 'bigsale') {
                                    ...
                                    } elseif($item['params']['saleout'] == 1 && !empty($item['style']['saleoutstyle'])) {
...
//                                        }
                            ...
                            elseif($item['id'] == 'hotarea')
                            {
                                $item['params']['bgimg'] = tomedia($item['params']['bgimg']);
                            }
                            else if($item['id'] == 'shortVideo') {
                                $item['params']['turl'] = tomedia($item['params']['turl']);
                                $item['params']['default'] = $_W['siteroot'] . '/addons/ewei_shopv2/static/images/';
                                $item['params']['shortPoster'] = tomedia($item['params']['shortPoster']);
                            }
                            else if($item['id'] == 'detailVideo') {
                                $item['params']['turl'] = tomedia($item['params']['turl']);
                                $item['params']['default'] = $_W['siteroot'] . '/addons/ewei_shopv2/static/images/';
                                $item['params']['shortPoster'] = tomedia($item['params']['shortPoster']);
                            }
        ...
        public function judge($type = null, $str = null){
            if(empty($type) || empty($str)) {
                ...
                } elseif(strexists($str, "/pages/member/cart")) {
                    return $order['cartnum'];
                } elseif(strexists($str, "/pages/member/favorite")) {
                    return $order['favorite'];
                } elseif(strexists($str, "/pages/sale/coupon/my")) {
                    return $order['coupon'];
                }
                return 0;
            }
...
array_walk($goods, function (&$goodsItem) {
                            ...
                            if ($goodsItem['ispresell'] && ($goodsItem['minprice'] > $goodsItem['presellprice']) && $in_time) {
                ...
                else if($item['params']['goodsdata'] > 2 && $mobile) {
...
//                    秒杀
        ...
        public function mediaData($item){
            ...
            if(is_array($item) && is_array($item['data']) && count($item['data']) > 0) {
                $newDatas = array();
                foreach ($item['data'] as $childid => $child) {
                    if(is_array($child)) {
         ...
         * @return mixed
         */
        public function getPageCode($params = array()){

            if(empty($params) || !is_array($params)) {
                return error(-1, '参数错误(params)');
            }
            if(empty($params['page'])) {
                return error(-1, '参数错误(page)');
            }
        ...
        public function getAuth(){
            ...
            $request = ihttp_get(EWEI_SHOPV2_AUTH_WXAPP . 'xcxapi/auth-xcx-info/validate?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'].$uri);
            if($request['code'] != 200) {
        ...
        public function diyMenu($menuid){
            ...
            return array(
                'type' => $data['params']['navstyle'],
                'navfloat' => $data['params']['navfloat'],
                'style' => $data['style'],
                'items' => $newMenu
            );
        }
        ...
        public function defaultMenu($page){
            ...
            if($page == 'shop') {
                ...
                if(p('commission') && $_W['shopset']['commission']['level'] > 0) {
                    $member = m('member')->getMember($_W['openid']);
        ...
        public function alipay_build($params, $config = array()){
            ...
            $arr = array(
                ...
                'version' => '1.0',
                'notify_url' => $_W['siteroot'] . 'addons/ewei_shopv2/payment/alipay/notify.php',
        ...
        function sendMessage($openid = '', $data = array(), $message_type = ''){
            ...
            if($message_type == TM_CYCELBUY_SELLER_DATE) {  //修改收货时间卖家消息通知
            ...
            } else if($message_type == TM_CYCELBUY_SELLER_ADDRESS) { //修改收货地址卖家消息通知
            ...
            } else if($message_type == TM_CYCELBUY_BUYER_DATE) { //修改收货时间买家消息通知
            ...
            } else if($message_type == TM_CYCELBUY_BUYER_ADDRESS) { //修改收货地址买家消息通知
        ...
        public function sendSubscribeMessage($openid, $msgdata, $type)
        {
            ...
            //windows系统需要用跟这个
            $res = ihttp_request($url,json_encode($data));
