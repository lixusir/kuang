...
//            }
            ...
            if(!empty($data)){
                foreach($data['items'] as $k => &$item){
                    if($item['id'] == 'detail_comment'){
                        unset($data['items'][$k]);
                    } else if ($item['id'] == 'shortVideo') {
                        if (empty($item['params']['shortVideourl']) && empty($item['params']['shortOrignUrl'])) {
                            show_json(0, '视频链接必选');
                        }
                        $item['params']['video_type'] = $item['style']['upType'];
                        if ($item['style']['upType'] == 1) {
                            $turl = parseTencentVideo($item['params']['shortVideourl']);
                            $item['params']['turl'] = $turl;
                        } else {
                            $item['params']['turl'] = tomedia($item['params']['shortOrignUrl']);
                        }
                    } else if ($item['id'] == 'detailVideo') {
                        if (empty($item['params']['detailVideourl']) && empty($item['params']['detailOrignUrl'])) {
                            show_json(0, '视频链接必选');
                        }
                        $item['params']['video_type'] = $item['style']['upType'];
                        if ($item['style']['upType'] == 1) {
                            $turl = parseTencentVideo($item['params']['detailVideourl']);
                            $item['params']['turl'] = $turl;
                        } else {
                            $item['params']['turl'] = tomedia($item['params']['detailOrignUrl']);
                        }
                    }
                }
                unset($item);
        ...
        $json = json_encode(array(
            ...
            'id'=>$id,
            'type'=>empty($page)? 2: $page['type'],
            'data'=>empty($page)? null: $page['data'],
            'diyadvs'=>$diyadvs
        ));
...
//            pdo_debug();die;
    ...
    public function selecticon2() {
        ...
        $list = array('', '首页', '全部分类', '商品列表', '购物车', '个人中心', '促销', '发现', '精选', '优选好货',
            '关注', '良品', '热卖', '新品', '人气', '砍价', '快速购买', '秒杀', '拼团', '发现2', '分类', '分销', '购物车2', '砍价2', '快速购买',
            '秒杀2', '拼团2', '首页2', '我的', '消息', '优惠券', '折扣', '折扣');
        unset($list[0]);
    ...
    public function get_code() {
        ...
        if ($type == 'index'){
            $ret = $this->model->getPageCode(array(
                'scene'=>'shopname=shopname',
