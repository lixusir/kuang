...
class Poster_EweiShopV2Page extends PluginWebPage {
	...
	public function main() {
        ...
        if(empty($keyword)){
            // 查出全部已使用
            $list_enabled = pdo_fetchall('SELECT id, title, thumb FROM '. tablename('ewei_shop_wxapp_poster'). ' WHERE uniacid=:uniacid AND status=1 ORDER BY displayorder ASC, id DESC', array(
                ':uniacid'=>$_W['uniacid']
            ));
    ...
    private function post() {
        ...
        if(!empty($id)){
            $item = pdo_fetch('SELECT * FROM '. tablename('ewei_shop_wxapp_poster'). ' WHERE id=:id AND uniacid=:uniacid LIMIT 1', array(
                ':id'=>$id,
                ':uniacid'=>$_W['uniacid']
            ));
            if(empty($item)){
        ...
        $json = json_encode(array(
            ...
            'data'=>!empty($item) && is_array($item['data'])? $item['data']: null
        ));
    ...
    public function status() {
        ...
        if(!empty($id)){
            ...
            pdo_update('ewei_shop_wxapp_poster', array(
                'status'=>$status,
                'displayorder'=>empty($status)? 0: time()
