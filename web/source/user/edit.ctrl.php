...
if ('edit_user_group' == $do) {
	...
	if ($vice_founder) {
		$group_info = pdo_get('users_founder_group', array('id' => $data['groupid']), array('package'));
	} else {
		$group_info = pdo_get('users_group', array('id' => $data['groupid']), array('package'));
	}
