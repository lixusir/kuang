...
<div id="js-system-module" ng-controller="installedCtrl" ng-cloak>
	<div class="search-box clearfix we7-margin-bottom ">
		...
		<div class="we7-form we7-flex" ng-show="checkUpgradeSuccess" ng-init="showMore = false">
			...
			<label class="checkbox-inline we7-margin-right" for="filter-2">
				新版本
			</label>
			<div class="more-filter-box color-default">
				<span ng-click="showMore = true">更多 <i class="wi wi-angle-down"></i></span>
				<div class="more-filter-list" ng-show="showMore">
					<div class="more-filter-item">
						<input type="checkbox" name="" ng-model="search.newVersion" id="filter-4" value="1">
						<label class="checkbox-inline we7-margin-right" for="filter-4">
							新版本
						</label>
					</div>
					<div class="more-filter-item">
						<input type="checkbox" name="" ng-model="search.serviceExpire" id="filter-5" value="1">
						<label class="checkbox-inline" for="filter-5">
							服务到期
						</label>
					</div>
					<div class="more-filter-item">
						<input type="checkbox" name="" ng-model="search.offSell" id="filter-6" value="1">
						<label class="checkbox-inline" for="filter-6">
							应用下架
						</label>
					</div>
					<div class="more-filter-item">
						<button class="btn btn-primary" ng-click="showMore = false;filter()">确定</button>
					</div>
				</div>
			</div>
		</div>
	...
	<form action="" method="get">
		<table class="table we7-table table-hover vertical-middle table-manage">
			...
			<tr ng-repeat="(key, module) in moduleList"  ng-if="key >= (pageindex - 1) * pagesize && key < pageindex * pagesize">
				...
				<td >
					<!--tag start-->
					<a ng-if="module.label" href="{{ module.label.url }}" target="_blank" class="module-tag-info"><i class="wi wi-info-list color-default"></i><span class="">{{ module.label.content }}</span></a>
					<!--tag end-->
				</td>
				<td class="vertical-middle table-manage-td" >
					<div class="link-group">
						<?php  if(permission_check_account_user('see_module_manage_system_ugrade')) { ?>
						<a href="<?php  echo url('module/manage-system/module_detail')?>&name={{ module.name }}&show=base" class="color-red del" ng-if="module.service_expire && module.system_shutdown != 2 && module.system_shutdown != 3">服务到期</a>
			...
			<tr style="height: 52px;" ng-if="!(moduleList | we7IsEmpty)"></tr>
		...
		<div class="select-all" ng-hide="!moduleList.length || (moduleList.length / pagesize <= 1)">
...
<div id="js-system-module-not_installed" ng-controller="notInstalledCtrl" ng-cloak>
	...
	<table class="table we7-table table-hover vertical-middle table-manage">
		...
		<tr ng-repeat="module in module_list">
			...
			<!--tag start-->
			<td class="" ng-if="!permission_check.see_module_manage_system_install">
				<div class="link-group" ng-if="module.label">
					<a href="{{ module.label.url }}" target="_blank" class="module-tag-info"><i class="wi wi-info-list color-default"></i><span class="">{{ module.label.content }}</span></a>
				</div>
			</td>
			<!--tag end-->
		</tr>
...
<div class="js-system-module-detail" ng-controller="detailCtrl" ng-cloak>
	...
	<div class="alert we7-page-alert" ng-if="moduleinfo.system_shutdown == 2 || moduleinfo.system_shutdown == 3">
		<p class="text-danger"><i class="wi wi-info-sign"></i>
			<span ng-show="!moduleinfo.can_update">该应用因违反平台规定已于<span class="color-red" ng-bind="moduleinfo.system_shutdown_delay_time"></span>下架，无法购买和更新版本</span></span>
	...
	<div class="btn-group we7-btn-group we7-margin-bottom">
		...
		<a  href="http://wo.w7.cc/#/work?site_id=<?php  echo $_W['setting']['site']['key'];?>&module_name=<?php  echo $module_name;?>" target="_blank" ng-class="{'active' : show == 'workorder'}" class="btn">工单系统</a>
		<?php  } ?>
	</div>
	...
	<table class="table we7-table table-hover vertical-middle" ng-show="show == 'upgrade'" ng-if="isFounder == 1 && upgradeInfo.from == 'cloud' && ((branch.displayorder >= upgradeInfo.site_branch.displayorder && moduleinfo.application_type != 2) || moduleinfo.application_type == 2)" ng-repeat="branch in upgradeInfo.branches">
		...
		<tr>
			...
			<td ng-if="moduleinfo.application_type != 2">{{ upgradeInfo.site_branch.id == branch.id ? moduleinfo.version : ''}}</td>
			<td ng-if="moduleinfo.application_type == 2">{{moduleinfo.version}}</td>
...
<script>
	require(['fileUploader'], function() {
		angular.module('moduleApp').value('config', {
            ...
            'downgradeUrl': "<?php  echo url('module/manage-system/downgrade')?>",
		});
