...
<div id="js-user-profile" ng-controller="UserProfileDisplay" ng-cloak>
	...
	<div class="base">
		...
		<table class="table we7-table table-hover table-form">
			...
			<col />
			<col width="100px" />
			<?php  if(empty($_W['isadmin'])) { ?>
			<tr>
				<th class="text-left" colspan="3">用户首页设置</th>
			</tr>
			<tr>
				<td class="table-label">登录后跳转链接</td>
	...
	<?php  if($user_type == PERSONAL_LIMIT_TYPE) { ?>
		<div class="account">
			<table class="table we7-table ">
				...
				<tr>
					<td class="color-gray">所属用户组：<?php  echo $group_info['name'];?></td>
					<td class="text-left">
						<span><?php  if($group_info['timelimit'] == 0 || !$group_info) { ?> 永久 <?php  } else { ?> <?php  echo $group_info['timelimit'];?> 天<?php  } ?></span>
					</td>
					<td class="text-right"></td>
				</tr>
				<tr>
					<td class="color-gray">附加权限</td>
					<td class="text-left"><?php  echo max(0, $extra_limit_info['timelimit']) . '天'?></td>
					<td class="text-right"></td>
				</tr>
			</table>
		</div>
